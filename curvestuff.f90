...
function findr_fft(theta)
  implicit none
  real *8::theta,r0
  real *8, dimension(:),allocatable::rhat,k,r2
  ...
  open(1,file='files/fft.txt')
    read(1,*) m
    allocate(rhat(m),r2(m),k(m))
    do i=1,m
      read(1,*) k(i),rhat(i),r2(i)
      ret = ret + (rhat(i))*cos(k(i)*theta) + r2(i)*sin(k(i)*theta)
    enddo
  close(1)
...
subroutine fftgen(n,args)
  ...
  real *8, dimension(:),allocatable::savedr,savedrc
  complex(c_double_complex),dimension(n)::rhat,rarc
  ...
  do i=1,n
    rarc(i) = cmplx(findr_fast(tarc(i),args),0.0d0)
  enddo
  ...
  call fftw_destroy_plan(plan)
  !change k array, put the second half before the first one (not
  !sure why we're doing this, ask Lee)
  ...
  m = 0
  do i=1,n
    if(real(rhat(i))>5e-12)then
      m = m+1
    endif
  enddo
  allocate(savedr(m),savedrc(m),savedk(m))

  j=1
  do i=1,n
    if(real(rhat(i))>5e-12)then
      savedr(j) = real(rhat(i))
      savedrc(j) = imag(rhat(i))
      savedk(j) = kf(i)
      j=j+1
    endif
  enddo
  ...
  write(1,*) m
  do i=1,m
    write(1,*) savedk(i),savedr(i)/n,savedrc(i)/n
  enddo
  close(1)
