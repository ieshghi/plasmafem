...
c       ... refine on sources only
        if(numpdad .le. nbox .and. level .ge. minlevel ) goto 2000
c
c       ... not a leaf node on sources
        if( numpdad .gt. nbox ) then
...
ccc     $     level .ge. minlevel ) goto 2000
c
c       
c       ... not a leaf node on sources or targets
c        if( numpdad .gt. nbox .or. numtdad .gt. nbox ) then
c        if( boxes(13,idad) .eq. 1 ) boxes(13,idad)=2
c        if( boxes(14,idad) .eq. 1 ) boxes(14,idad)=2
c        endif
c
c
        ii=boxes(2,idad)
